using Avalonia;
using Avalonia.OpenGL;
using Avalonia.OpenGL.Controls;
using Silk.NET.OpenGL;
using System;
using System.Diagnostics;
using System.Numerics;

namespace stewart_platform
{
    public class PlatformView3D : OpenGlControlBase
    {
        private GL? _gl;
        private uint _vao;
        private uint _vbo;
        private uint _shaderProgram;
        private int _mvpLocation;
        private int _colorLocation;

        public StewartPlatform? Platform { get; set; }

        // Smart Vertex Shader Generator based on OS Environment
        private string GetVertexShader(bool isES)
        {
            string version = isES ? "#version 100\n" : "#version 120\n";
            return version + @"
                attribute vec3 aPos;
                uniform mat4 uMVP;
                void main() {
                    gl_Position = uMVP * vec4(aPos, 1.0);
                }";
        }

        // Smart Fragment Shader Generator based on OS Environment
        private string GetFragmentShader(bool isES)
        {
            string version = isES ? "#version 100\n" : "#version 120\n";
            string precision = isES ? "precision mediump float;\n" : "";
            return version + precision + @"
                uniform vec4 uColor;
                void main() {
                    gl_FragColor = uColor;
                }";
        }

        protected override unsafe void OnOpenGlInit(GlInterface glInterface)
        {
            base.OnOpenGlInit(glInterface);
            _gl = GL.GetApi(glInterface.GetProcAddress);

            _gl.ClearColor(0.15f, 0.15f, 0.15f, 1.0f);
            _gl.Enable(EnableCap.DepthTest);
            _gl.LineWidth(3.0f);

            // Detect if running on Windows ANGLE (OpenGL ES) or Native Linux/Pi (Desktop GL)
            bool isES = glInterface.Version != null && glInterface.Version.Contains("ES");

            // 1. Compile Shaders safely
            uint vertexShader = CompileShader(ShaderType.VertexShader, GetVertexShader(isES));
            uint fragmentShader = CompileShader(ShaderType.FragmentShader, GetFragmentShader(isES));

            _shaderProgram = _gl.CreateProgram();
            _gl.AttachShader(_shaderProgram, vertexShader);
            _gl.AttachShader(_shaderProgram, fragmentShader);

            // CRITICAL: Bind the attribute location manually before linking 
            // because old OpenGL / GLES doesn't support "layout(location = 0)" in the shader text
            _gl.BindAttribLocation(_shaderProgram, 0, "aPos");

            _gl.LinkProgram(_shaderProgram);

            // Error Checking for Linking
            string programLog = _gl.GetProgramInfoLog(_shaderProgram);
            if (!string.IsNullOrWhiteSpace(programLog))
            {
                Debug.WriteLine($"[OpenGL] Program Link Error: {programLog}");
            }

            _gl.DeleteShader(vertexShader);
            _gl.DeleteShader(fragmentShader);

            _mvpLocation = _gl.GetUniformLocation(_shaderProgram, "uMVP");
            _colorLocation = _gl.GetUniformLocation(_shaderProgram, "uColor");

            _vao = _gl.GenVertexArray();
            _vbo = _gl.GenBuffer();
        }

        private unsafe uint CompileShader(ShaderType type, string source)
        {
            uint shader = _gl!.CreateShader(type);
            _gl.ShaderSource(shader, source);
            _gl.CompileShader(shader);

            // Output any graphics card compilation errors to Visual Studio
            string infoLog = _gl.GetShaderInfoLog(shader);
            if (!string.IsNullOrWhiteSpace(infoLog))
            {
                Debug.WriteLine($"[OpenGL] Shader Compile Error ({type}): {infoLog}");
            }

            return shader;
        }

        protected override void OnOpenGlDeinit(GlInterface glInterface)
        {
            _gl?.DeleteVertexArray(_vao);
            _gl?.DeleteBuffer(_vbo);
            _gl?.DeleteProgram(_shaderProgram);
            _gl?.Dispose();
            base.OnOpenGlDeinit(glInterface);
        }

        protected override unsafe void OnOpenGlRender(GlInterface glInterface, int fb)
        {
            // Guard against divide-by-zero on launch
            if (_gl == null || Bounds.Height == 0 || Bounds.Width == 0) return;

            _gl.Clear((uint)(ClearBufferMask.ColorBufferBit | ClearBufferMask.DepthBufferBit));
            _gl.Viewport(0, 0, (uint)Bounds.Width, (uint)Bounds.Height);

            // Don't draw if the math engine isn't linked yet
            if (Platform == null) return;

            _gl.UseProgram(_shaderProgram);

            // Setup Camera
            float aspectRatio = (float)(Bounds.Width / Bounds.Height);

            // Look from Y=-350 (front), Z=200 (up) towards the center (0,0,0)
            Matrix4x4 view = Matrix4x4.CreateLookAt(new Vector3(0, -350, 200), Vector3.Zero, Vector3.UnitZ);
            Matrix4x4 projection = Matrix4x4.CreatePerspectiveFieldOfView(MathF.PI / 4f, aspectRatio, 0.1f, 1000f);
            Matrix4x4 mvp = view * projection;

            // Pass Matrix to GPU. "false" keeps the correct layout for System.Numerics
            float* mvpPtr = (float*)&mvp;
            _gl.UniformMatrix4(_mvpLocation, 1, false, mvpPtr);

            if (_vao != 0) _gl.BindVertexArray(_vao);

            // --- Draw Components ---

            // Draw Base (Blue Hexagon)
            DrawLineLoop(Platform.BasePoints, new Vector4(0.2f, 0.6f, 1.0f, 1.0f));

            // Draw Top Platform (Green Hexagon)
            DrawLineLoop(Platform.PlatformPoints, new Vector4(0.2f, 1.0f, 0.2f, 1.0f));

            // Draw Horns and Rods
            for (int i = 0; i < 6; i++)
            {
                // Horn (Yellow Line)
                DrawLine(Platform.BasePoints[i], Platform.HornEndPoints[i], new Vector4(1.0f, 1.0f, 0.0f, 1.0f));

                // Rod (Red Line)
                DrawLine(Platform.HornEndPoints[i], Platform.PlatformPoints[i], new Vector4(1.0f, 0.2f, 0.2f, 1.0f));
            }

            if (_vao != 0) _gl.BindVertexArray(0);
        }

        private unsafe void DrawLineLoop(Vector3[] points, Vector4 color)
        {
            _gl!.Uniform4(_colorLocation, color.X, color.Y, color.Z, color.W);
            _gl.BindBuffer(BufferTargetARB.ArrayBuffer, _vbo);

            fixed (Vector3* ptr = points)
            {
                _gl.BufferData(BufferTargetARB.ArrayBuffer, (nuint)(points.Length * sizeof(Vector3)), ptr, BufferUsageARB.DynamicDraw);
            }

            _gl.VertexAttribPointer(0, 3, VertexAttribPointerType.Float, false, (uint)(3 * sizeof(float)), null);
            _gl.EnableVertexAttribArray(0);
            _gl.DrawArrays(PrimitiveType.LineLoop, 0, (uint)points.Length);
        }

        private unsafe void DrawLine(Vector3 p1, Vector3 p2, Vector4 color)
        {
            _gl!.Uniform4(_colorLocation, color.X, color.Y, color.Z, color.W);
            Vector3[] line = new Vector3[] { p1, p2 };

            _gl.BindBuffer(BufferTargetARB.ArrayBuffer, _vbo);
            fixed (Vector3* ptr = line)
            {
                _gl.BufferData(BufferTargetARB.ArrayBuffer, (nuint)(2 * sizeof(Vector3)), ptr, BufferUsageARB.DynamicDraw);
            }

            _gl.VertexAttribPointer(0, 3, VertexAttribPointerType.Float, false, (uint)(3 * sizeof(float)), null);
            _gl.EnableVertexAttribArray(0);
            _gl.DrawArrays(PrimitiveType.Lines, 0, 2);
        }

        public void Redraw() => RequestNextFrameRendering();
    }
}